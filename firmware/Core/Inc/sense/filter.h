...
typedef struct {
    float threshold;         /* Minimum amplitude to consider a peak */
    int refractory_period;   /* Number of samples to skip after detecting a peak */
    int refractory_count;    /* Countdown for refractory period */
    float candidate;         /* Candidate peak value */
    int tracking;            /* Flag indicating if we're tracking a potential peak (1) or not (0) */
} PeakDetector;
