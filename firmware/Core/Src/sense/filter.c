...
CHEBandPass *create_che_band_pass_filter(int n, FTR_PRECISION epsilon, FTR_PRECISION s, FTR_PRECISION f_lower,
                                         FTR_PRECISION f_upper) {
...
CHEBandStop *create_che_band_stop_filter(int n, FTR_PRECISION epsilon, FTR_PRECISION s, FTR_PRECISION f_lower,
                                         FTR_PRECISION f_upper) {
...
FTR_PRECISION bw_band_pass(BWBandPass *filter, FTR_PRECISION x) {
    int i;
    for (i = 0; i < filter->n; ++i) {
        filter->w0[i] = filter->d1[i] * filter->w1[i] + filter->d2[i] * filter->w2[i] + filter->d3[i] * filter->w3[i] +
...
FTR_PRECISION bw_band_stop(BWBandStop *filter, FTR_PRECISION x) {
    ...
    for (i = 0; i < filter->n; ++i) {
        filter->w0[i] = filter->d1[i] * filter->w1[i] + filter->d2[i] * filter->w2[i] + filter->d3[i] * filter->w3[i] +
                        filter->d4[i] * filter->w4[i] + x;
        x = filter->A[i] * (filter->w0[i] - filter->r * filter->w1[i] + filter->s * filter->w2[i] - filter->r * filter->
                            w3[i] + filter->w4[i]);
        filter->w4[i] = filter->w3[i];
        filter->w3[i] = filter->w2[i];
        filter->w2[i] = filter->w1[i];
...
void spike_filter_upward(FTR_PRECISION *input, int size, FTR_PRECISION *output, FTR_PRECISION strength) {
    ...
    for (int i = 1; i < size - 1; i++) {
        FTR_PRECISION val = input[i - 1] * SPIKE_KERNEL[0] + input[i] * SPIKE_KERNEL[1] + input[i + 1] * SPIKE_KERNEL[
                                2];
...
void PeakDetector_Init(PeakDetector *det, float threshold, int refractory_period) {
    det->threshold = threshold;
    det->refractory_period = refractory_period;
    det->refractory_count = 0;
    det->candidate = 0.0f;
    det->tracking = 0;
}

/*
   Process a new sample and update the peak detection state.
   Returns 1 if a peak is detected at this sample, 0 otherwise.
*/
int PeakDetector_Process(PeakDetector *det, float sample) {
    int peak_detected = 0;

    /* If we are in the refractory period, decrement counter and do nothing */
    if (det->refractory_count > 0) {
        det->refractory_count--;
        return 0;
    }

    if (!det->tracking) {
        /* Not currently tracking a peak. If the sample exceeds the threshold, start tracking. */
        if (sample > det->threshold) {
            det->candidate = sample;
            det->tracking = 1;
        }
    } else {
        /* Currently tracking a potential peak. Update candidate if the sample is rising. */
        if (sample > det->candidate) {
            det->candidate = sample;
        } else {
            /* Once the signal falls, we declare the candidate as a peak.
               Additional conditions can be added (e.g., requiring a minimum drop)
               to make detection more robust. */
            if (det->candidate > det->threshold) {
                peak_detected = 1;
                /* Optionally, you can do something with the detected peak value here:
                   float detectedPeak = det->candidate; */
            }
            /* Reset tracking and set the refractory period to avoid multiple detections */
            det->tracking = 0;
            det->candidate = 0.0f;
            det->refractory_count = det->refractory_period;
        }
    }
