...
void weather_draw(void) {
    ...
    draw_text(buf, CENTER_X, 100, &montserrat_reg, COLOR_WHITE, 1.2, true);
    ...
    snprintf(buf, sizeof(buf), "H: %.1f°%c  L: %.1f°%c",
             ...
             g_app_data.settings.metric ? 'C' : 'F');
    draw_text(buf, CENTER_X, 160, &montserrat_reg, COLOR_WHITE, 0.6, true);
    
    // humidity 
    snprintf(buf, sizeof(buf), "Humidity: %d%%", g_app_data.weather.humidity);
    draw_text(buf, CENTER_X, 190, &montserrat_reg, COLOR_WHITE, 0.8, true);
    
    // button desc
    draw_text("Press any button for menu", CENTER_X, LCD_1IN28_HEIGHT - 20, 
              &montserrat_reg, COLOR_WHITE, 0.6, true);
}
