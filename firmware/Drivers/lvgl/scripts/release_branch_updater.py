...
def main():
    ...
    arg_parser.add_argument("--port-urls-path",
                            default=os.path.join(os.path.dirname(__file__), "release_branch_updater_port_urls.txt"))
        ...
        for lvgl_branch in lvgl_release_branches + [lvgl_default_branch]:
            ...
            if port_branch in port_release_branches:
                ...
                if create_from is None:
                    ...
                    print(LOG,
                          "... this port has no major from which to create the minor. one must be created manually. continuing to next.")
                    continue
            ...
            out = subprocess.check_output(("find", ".", "-name", "lv_conf.defaults", "-print", "-quit"),
                                          cwd=port_clone_tmpdir)
            ...
            if port_lv_conf_defaults is None:
                ...
                out = subprocess.check_output(("find", ".", "-name", "lv_conf.h", "-print", "-quit"),
                                              cwd=port_clone_tmpdir)
                ...
                if port_lv_conf_h is None:
                    ...
                    subprocess.check_call((sys.executable, os.path.join(lvgl_path, "scripts/generate_lv_conf.py"),
                                           "--defaults",
                                           os.path.abspath(os.path.join(port_clone_tmpdir, port_lv_conf_defaults)),
                                           "--config",
                                           os.path.abspath(os.path.join(port_clone_tmpdir, port_lv_conf_h)),))
