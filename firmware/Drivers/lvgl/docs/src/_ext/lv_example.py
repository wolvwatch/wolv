...
class LvExample(Directive):
    ...
    def embed_code(self, example_file, example_path, language, buttons={}):
        ...
        for text, url in buttons.items():
            paragraph_node.append(nodes.raw(
                text=f"<a class='lv-example-link-button' onclick=\"event.stopPropagation();\" href='{url}'>{text}</a>",
                format='html'))
    ...
    def run(self):
        ...
        layout_node = nodes.raw(text=f"<div class='lv-example-container'>{iframe_html}{description_html}</div>",
                                format='html')
